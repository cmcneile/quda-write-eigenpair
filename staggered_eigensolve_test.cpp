...
/***
    From ../generic/io_scidac.c
 ***/

/***  from MILC code (no parallel IO)  ****/
...
int qio_node_number(const int x[]){
  //  return node_number(x[0],x[1],x[2],x[3]);  // MILC code
return QMP_get_node_number_from(x);
}
#else
int qio_node_number(const int x[]){
  static int ss = 1;
 ...
 if( ss )
   {
     ...
     void *fdata = (void *) &map_data;
     int  nn = lex_rank_from_coords(x,fdata) ;
...
int qio_num_sites(int node){
  ...
  int volume = xdim*ydim*zdim*tdim ; /**   OK ???   **/

  return  volume / comm_size() ;
...
#define LATDIM 4
  ...
  ************/
  ...
  /**  worry whether this is global lattice dimension in QUDA  **/
  lattice_size[0] = xdim;
  lattice_size[1] = ydim;
  lattice_size[2] = zdim;
  lattice_size[3] = tdim;
  ...
  /*  layout->node_index      = qio_node_index;   were the same in the MILC code   *****/
  layout->node_index      =  qio_node_number ;
  layout->get_coords      = qio_get_coords;
  layout->num_sites       = qio_num_sites; /**  check **/
  layout->latsize         = lattice_size;
  layout->latdim          = LATDIM;
  layout->volume          = volume;
  /**  layout->sites_on_node   = sites_on_node;   **/
  layout->sites_on_node   = qio_num_sites(0) ; /**  ist correct **/


  /**  layout->this_node       = this_node;  **/
  layout->this_node       = comm_rank() ; 
...
open_ks_eigen_outfile(char *filename, int Nvecs, int volfmt, int serpar){
  ...
  //  build_qio_layout(&layout);  /** more work ./generic/io_scidac.c   **/

  /* Define the I/O system */
  build_qio_filesystem(&fs);   /** more work ./generic/io_scidac.c   **/
  ...
  /*  outfile = open_scidac_output(filename, volfmt, serpar, QIO_ILDGNO, NULL,
      &layout, &fs, filexml); */  /** more work ./generic/io_scidac.c   **/  
...
#if 0
// need to do something with eigVec
int
...
int
write_ks_eigenvector(QIO_Writer *outfile, double *eigVec, double eigVal, 
                     double resid)
{
  int status;
  char *xml;
